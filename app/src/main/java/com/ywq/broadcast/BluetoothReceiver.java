...
public class BluetoothReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (btDevice==null|| TextUtils.isEmpty(btDevice.getName())){
            return;
        }
        if (BluetoothDevice.ACTION_FOUND.equals(action)) {  //发现设备
            Log.e("xhw", "发现设备 [" + btDevice.getName() + "]" + ":" + btDevice.getAddress());

            if (btDevice.getName().contains("锤子"))//锤子设备如果有多个，第一个搜到的那个会被尝试。
            {
                if (btDevice.getBondState() == BluetoothDevice.BOND_NONE) {

                    Log.e("xhw", "attemp to bond:" + "[" + btDevice.getName() + "]");
                    try {
        ...
        {
            Log.e("xhw","action2="+ action);
            if (btDevice.getName().contains("锤子")) {
                Log.e("here", "OKOKOK");
